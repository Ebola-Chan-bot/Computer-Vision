...
%[text] [Lines,Texts]=ComputerVision.PLine(Descriptors);
%[text] ```
%[text] ## 示例
%[text] ```matlabCodeExample
...
%[text] box off;
%[text] ComputerVision.PLine(table(EBs(1),EBs(end),1,2,"n.s.",'VariableNames',["ObjectA","ObjectB","IndexA","IndexB","Text"]));
%[text] xlabel('Time from 💡(:)💧(|)');
...
%[text] ```
%[text] ![](text:image:31bd)
%[text] ## 输入参数
%[text] ### Descriptors
%[text] tabular，一行一个标识，包含以下列：
...
%[text] 如果ObjectA和ObjectB不同：如果IndexA和IndexB指定其一，则另一个默认与指定值相同；否则报错。
%[text] ### RedundantDistance
%[text] (1,1)=0，多个P值线、P值线和文本之间的留白距离，以图上标尺为单位。
%[text] ## 返回值
%[text] Lines(:,1)matlab.graphics.primitive.Line，返回标识线以便后期手动修改属性
%[text] Texts(:,1)matlab.graphics.primitive.Text，返回标识文本以便后期手动修改属性
%[text] **See also** [matlab.graphics.chart.primitive.Bar](<matlab:doc matlab.graphics.chart.primitive.Bar>) [matlab.graphics.chart.primitive.ErrorBar](<matlab:doc matlab.graphics.chart.primitive.ErrorBar>) [matlab.graphics.primitive.Line](<matlab:doc matlab.graphics.primitive.Line>) [matlab.graphics.chart.primitive.Scatter](<matlab:doc matlab.graphics.chart.primitive.Scatter>) [matlab.graphics.primitive.Text](<matlab:doc matlab.graphics.primitive.Text>) [ComputerVision.PLineRetune](<matlab:doc ComputerVision.PLineRetune>)
function [Lines,Texts]=PLine(Descriptors,RedundantDistance)
arguments
	Descriptors
	RedundantDistance=0
end
...
for D=1:NumPLines
	...
	if ObjectA==ObjectB
			...
			if IndexB==0
				if isa(ObjectA,'matlab.graphics.chart.primitive.Scatter')
					...
					if numel(XData)==2
						...
						ComputerVision.Exception.Invalid_descriptor.Throw({'无法确认单一Scatter集内的关键点';Descriptors(D,:)});
					end
				else
					Index=[1,numel(XData)];
				end
			else
				ComputerVision.Exception.Invalid_descriptor.Throw({'不允许为相同对象指定单一索引';Descriptors(D,:)});
			end
		else
			if IndexB==0
				ComputerVision.Exception.Invalid_descriptor.Throw({'不允许为相同对象指定单一索引';Descriptors(D,:)});
		...
		if IndexA==0
			if IndexB==0
				if VerticalPLine
					...
					ComputerVision.Exception.Invalid_descriptor.Throw({'不同的非Line对象至少需要指定一个索引';Descriptors(D,:)});
				end
			else
				IndexA=IndexB;
			end
		...
		if isa(ObjectA,'matlab.graphics.chart.primitive.ErrorBar')
			if XData(1)==XData(2)
				if YData(1)==YData(2)
					ComputerVision.Exception.Invalid_descriptor.Throw({'重合的点';Descriptors(D,:)});
				else
	...
	XRuler=Ax.XAxis;
	if anymissing(YData)
		ComputerVision.Exception.PLine_bound_to_missing_value.Throw(Descriptors(D,:));
	end
	if VerticalPLine
...
if ~VerticalPLine
	...
	ComputerVision.PLineRetune(Lines,Texts,RedundantDistance);
end
end
